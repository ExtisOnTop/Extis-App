...
class PlayerActions extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, ref) {
    ...
    final isLocalTrack = playlist.activeTrack is ExtisLocalTrackObject;
    ref.watch(downloadManagerProvider);
    final downloader = ref.watch(downloadManagerProvider.notifier);
    final isInQueue = useMemoized(() {
      if (playlist.activeTrack is! ExtisFullTrackObject) return false;
      final downloadTask =
      ...
      children: [
        if (showQueue)
          Tooltip(
            ...
            child: IconButton.ghost(
              icon: const Icon(ExtisIcons.queue),
        ...
        if (!isLocalTrack)
          Tooltip(
            ...
            child: IconButton.ghost(
              enabled: playlist.activeTrack != null,
              icon: const Icon(ExtisIcons.alternativeRoute),
              onPressed: () {
        ...
        if (!kIsWeb && !isLocalTrack)
          if (isInQueue)
            ...
            Tooltip(
              ...
              child: IconButton.ghost(
                icon: Icon(
                  isDownloaded ? ExtisIcons.done : ExtisIcons.download,
                ),
                onPressed: playlist.activeTrack != null
                    ? () => downloader.addToQueue(
                        playlist.activeTrack! as ExtisFullTrackObject)
                    : null,
              ),
            ),
