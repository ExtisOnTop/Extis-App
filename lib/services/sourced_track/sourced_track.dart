...
class SourcedTrack extends BasicSourcedTrack {
  ...
  static Future<SourcedTrack> fetchFromTrack({
    required ExtisFullTrackObject query,
    required Ref ref,
  }) async {
    ...
    final item = ExtisAudioSourceMatchObject.fromJson(
  ...
  static List<ExtisAudioSourceMatchObject> rankResults(
    List<ExtisAudioSourceMatchObject> results,
  ...
  static Future<List<ExtisAudioSourceMatchObject>> fetchSiblings({
    required ExtisFullTrackObject query,
    required Ref ref,
  }) async {
    final audioSource = await ref.read(audioSourcePluginProvider.future);
    ...
    final videoResults = <ExtisAudioSourceMatchObject>[];
  ...
  Future<SourcedTrack?> swapWithSibling(
    ExtisAudioSourceMatchObject sibling,
  ) async {
  ...
  Future<SourcedTrack> refreshStream() async {
    ...
    List<ExtisAudioSourceStreamObject> validStreams = [];
  ...
  ExtisAudioSourceStreamObject? getStreamOfQuality(
    ExtisAudioSourceContainerPreset preset,
    int qualityIndex,
  ) {
    ...
    final exactMatch = sources.firstWhereOrNull(
      (source) {
        ...
        if (quality case ExtisAudioLosslessContainerQuality()) {
          return source.sampleRate == quality.sampleRate &&
              source.bitDepth == quality.bitDepth;
        } else {
          return source.bitrate ==
              (quality as ExtisAudioLossyContainerQuality).bitrate;
        }
      },
    );
    ...
    return sources.where((source) {
      ...
      if (quality is ExtisAudioLosslessContainerQuality) {
  ...
  ExtisAudioSourceContainerPreset? get qualityPreset {
