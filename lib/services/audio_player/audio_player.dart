...
class ExtisMedia extends mk.Media {
  ...
  ExtisMedia(this.track)
      : assert(
          track is ExtisLocalTrackObject || track is ExtisFullTrackObject,
        ...
        super(
          track is ExtisLocalTrackObject
              ? track.path
              : "http://$_host:$serverPort/stream/${track.id}",
          extras: track.toJson(),
        );

  factory ExtisMedia.media(Media media) {
    assert(media.extras != null, "[Media] must have extra metadata set");
    return ExtisMedia(ExtisTrackObject.fromJson(media.extras!));
  }
}
