  ...
  Future<void> action(
    ...
    switch (value) {
      ...
      case TrackOptionValue.delete:
        await File((track as ExtisLocalTrackObject).path).delete();
      ...
      case TrackOptionValue.details:
        if (track is! ExtisFullTrackObject) break;
        showDialog(
          context: context,
          builder: (context) => ConstrainedBox(
            constraints: const BoxConstraints(maxWidth: 400),
            child: TrackDetailsDialog(track: track as ExtisFullTrackObject),
          ),
        );
        break;
      case TrackOptionValue.download:
        if (track is ExtisLocalTrackObject) break;
        downloadManager.addToQueue(track as ExtisFullTrackObject);
        break;
...
final trackOptionActionsProvider =
    Provider.family<TrackOptionsActions, ExtisTrackObject>(
  (ref, track) => TrackOptionsActions(ref, track),
);

final trackOptionsStateProvider =
    Provider.family<TrackOptionFlags, ExtisTrackObject>((ref, track) {
  ...
  final isInDownloadQueue = playlist.activeTrack == null ||
          playlist.activeTrack! is ExtisLocalTrackObject
      ? false
      : const [
