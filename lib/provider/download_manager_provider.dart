...
class DownloadTask {
  final ExtisFullTrackObject track;
...
class DownloadManagerNotifier extends Notifier<List<DownloadTask>> {
  ...
  void addToQueue(ExtisFullTrackObject track) {
  ...
  void addAllToQueue(List<ExtisFullTrackObject> tracks) {
    state = [
      ...state,
  ...
  void retry(ExtisFullTrackObject track) {
  ...
  void cancel(ExtisFullTrackObject track) {
    if (state.firstWhereOrNull((e) => e.track.id == track.id)?.status ==
  ...
  void _setStatus(ExtisFullTrackObject track, DownloadStatus status) {
    state = state.map((e) {
