...
class SyncedLyricsNotifier
    extends FamilyAsyncNotifier<SubtitleSimple, ExtisTrackObject?> {
  ExtisTrackObject get _track => arg!;
  ...
  Future<SubtitleSimple> getLRCLibLyrics() async {
    ...
    final res = await globalDio.getUri(
      ...
      options: Options(
        headers: {
          "User-Agent":
              "Extis v${packageInfo.version} (https://github.com/KRTirtho/extis)"
        },
...
final syncedLyricsProvider = AsyncNotifierProviderFamily<SyncedLyricsNotifier,
    SubtitleSimple, ExtisTrackObject?>(
  () => SyncedLyricsNotifier(),
);

final syncedLyricsMapProvider =
    FutureProvider.family((ref, ExtisTrackObject? track) async {
