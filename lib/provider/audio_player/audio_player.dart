...
class AudioPlayerNotifier extends Notifier<AudioPlayerState> {
  ...
  void _assertAllowedTracks(Iterable<ExtisTrackObject> tracks) {
    assert(
      tracks.every(
        (track) =>
            track is ExtisFullTrackObject || track is ExtisLocalTrackObject,
      ),
      'All tracks must be either ExtisFullTrackObject or ExtisLocalTrackObject',
    );
  }

  void _assertAllowedTrack(ExtisTrackObject tracks) {
    assert(
      tracks is ExtisFullTrackObject || tracks is ExtisLocalTrackObject,
      'Track must be either ExtisFullTrackObject or ExtisLocalTrackObject',
    );
  }
    ...
    if (playerState == null) {
      await database.into(database.audioPlayerStateTable).insert(
            AudioPlayerStateTableCompanion.insert(
              ...
              collections: <String>[],
              tracks: const Value(<ExtisTrackObject>[]),
  ...
  build() {
    final subscriptions = [
      ...
      audioPlayer.playlistStream.listen((playlist) async {
        try {
          final tracks =
              playlist.medias.map((e) => ExtisMedia.media(e).track).toList();
  ...
  Future<void> addTracksAtFirst(
    Iterable<ExtisTrackObject> tracks, {
  ...
  Future<void> addTrack(ExtisTrackObject track) async {
    ...
    await audioPlayer.addTrack(ExtisMedia(track));
  ...
  Future<void> addTracks(Iterable<ExtisTrackObject> tracks) async {
    _assertAllowedTracks(tracks);
  ...
  bool _compareTracks(ExtisTrackObject a, ExtisTrackObject b) {
    if (a.runtimeType != b.runtimeType) {
      return false;
    }

    return a is ExtisLocalTrackObject && b is ExtisLocalTrackObject
        ? a.path == b.path
        : a.id == b.id;
  }
  ...
  Future<void> load(
    ...
    if (intendedActiveTrack.track is! ExtisLocalTrackObject) {
      ref.read(
        sourcedTrackProvider(
          intendedActiveTrack.track as ExtisFullTrackObject,
        ).future,
      );
    }
  ...
  Future<void> swapActiveSource() async {
    if (state.tracks.isEmpty || state.activeTrack is! ExtisFullTrackObject) {
      return;
    }
  ...
  Future<void> jumpToTrack(ExtisTrackObject track) async {
    final index =
