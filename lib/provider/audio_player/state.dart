...
class AudioPlayerState with _$AudioPlayerState {
  ...
  factory AudioPlayerState._inner({
    ...
    @Default([]) List<ExtisTrackObject> tracks,
  }) = _AudioPlayerState;
  ...
  factory AudioPlayerState({
    ...
    required List<String> collections,
    int currentIndex = 0,
    List<ExtisTrackObject> tracks = const [],
  }) {
    assert(
      tracks.every((track) =>
  ...
  ExtisTrackObject? get activeTrack {
    if (currentIndex < 0 || currentIndex >= tracks.length) return null;
    return tracks[currentIndex];
  }

  bool containsTrack(ExtisTrackObject track) {
    return tracks.isNotEmpty &&
        tracks.any(
          (t) =>
              t is ExtisLocalTrackObject && track is ExtisLocalTrackObject
                  ? t.path == track.path
                  : t.id == track.id,
        );
  }

  bool containsTracks(List<ExtisTrackObject> tracks) {
