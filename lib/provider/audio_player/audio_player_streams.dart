...
class AudioPlayerStreamListeners {
  ...
  StreamSubscription subscribeToScrobbleChanged() {
    ...
    return audioPlayer.positionStream.listen((position) async {
      try {
        final uid = audioPlayerState.activeTrack is ExtisLocalTrackObject
            ? (audioPlayerState.activeTrack as ExtisLocalTrackObject).path
        ...
        if (activeTrack.artists.any((a) => a.images == null)) {
          ...
          activeTrack = activeTrack.copyWith(
            artists: artists
                .map((e) => ExtisSimpleArtistObject.fromJson(e.toJson()))
                .toList(),
          );
        }
    ...
    return audioPlayer.positionStream.listen((event) async {
      ...
      try {
        ...
        if (nextTrack == null ||
            ...
            nextTrack is ExtisLocalTrackObject) {
          return;
        }

        try {
          await ref.read(
            sourcedTrackProvider(nextTrack as ExtisFullTrackObject).future,
          );
        } finally {
          lastTrack = nextTrack.id;
        }
