...
class MetadataPluginSavedTracksNotifier
    extends AutoDisposePaginatedAsyncNotifier<ExtisFullTrackObject> {
  ...
  Future<void> addFavorite(List<ExtisTrackObject> tracks) async {
    if (state.value == null) {
      return;
    }
    ...
    state = AsyncData(
      state.value!.copyWith(
        items: [
          ...tracks.whereType<ExtisFullTrackObject>(),
          ...state.value!.items
        ],
      ),
    );
  ...
  Future<void> removeFavorite(List<ExtisTrackObject> tracks) async {
    if (state.value == null) {
      return;
    }
...
final metadataPluginSavedTracksProvider = AutoDisposeAsyncNotifierProvider<
    ...
    ExtisPaginationResponseObject<ExtisFullTrackObject>>(
  () => MetadataPluginSavedTracksNotifier(),
);
