...
class MetadataPluginScrobbleNotifier
    extends Notifier<StreamController<ExtisTrackObject>?> {
  @override
  build() {
    ...
    final controller = StreamController<ExtisTrackObject>.broadcast();
    ...
    final subscription = controller.stream.listen((event) async {
      try {
        await metadataPlugin.valueOrNull?.core.scrobble({
          ...
          "isrc": event is ExtisFullTrackObject ? event.isrc : null,
        });
      } catch (e, stack) {
  ...
  void scrobble(ExtisTrackObject track) {
    state?.add(track);
  }
}

final metadataPluginScrobbleProvider = NotifierProvider<
    MetadataPluginScrobbleNotifier, StreamController<ExtisTrackObject>?>(
