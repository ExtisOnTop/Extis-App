...
final localTracksProvider =
    FutureProvider<Map<String, List<ExtisLocalTrackObject>>>((ref) async {
  try {
    if (kIsWeb) return {};
    final Map<String, List<ExtisLocalTrackObject>> libraryToTracks = {};
    ...
    for (final location in [
      ...
      final tracksFromMetadata = filesWithMetadata
          .map(
            (fileWithMetadata) => ExtisTrackObject.localTrackFromFile(
              fileWithMetadata.file,
              metadata: fileWithMetadata.metadata,
              art: fileWithMetadata.art,
            ) as ExtisLocalTrackObject,
          )
          .toList();
