...
class DiscordNotifier extends AsyncNotifier<void> {
  ...
  Future<void> updatePresence(ExtisTrackObject track) async {
    if (!kIsDesktop) return;
    ...
    await FlutterDiscordRPC.instance.setActivity(
      activity: RPCActivity(
        ...
        assets: RPCAssets(
          largeImage:
              track.album.images.firstOrNull?.url ?? "extis-logo-foreground",
          largeText: track.album.name,
          smallImage: "extis-logo-foreground",
          smallText: "Extis",
        ),
        buttons: [
          RPCButton(
            label: "Listen on Extis",
