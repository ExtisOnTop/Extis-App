...
UseActionCallbacks useActionCallbacks(WidgetRef ref) {
    ...
    try {
      ...
      if (isRemoteDevice) {
        ...
        await remotePlayback.load(
          options.collection is ExtisSimpleAlbumObject
              ? WebSocketLoadEventData.album(
                  tracks: allTracks,
                  collection: options.collection as ExtisSimpleAlbumObject,
                  initialIndex: Random().nextInt(allTracks.length))
              : WebSocketLoadEventData.playlist(
                  tracks: allTracks,
                  collection: options.collection as ExtisSimplePlaylistObject,
                  initialIndex: Random().nextInt(allTracks.length),
                ),
        );
        ...
        if (options.collection is ExtisSimpleAlbumObject) {
          historyNotifier
              .addAlbums([options.collection as ExtisSimpleAlbumObject]);
        } else {
  ...
  final onAddToQueue = useCallback(() {
    ...
    if (options.collection is ExtisSimpleAlbumObject) {
      ...
      historyNotifier
          .addPlaylists([options.collection as ExtisSimplePlaylistObject]);
    }
    if (!context.mounted) return;
