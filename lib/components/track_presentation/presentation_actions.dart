...
ToastOverlay showToastForAction(
  ...
  final message = switch (action) {
    "download" => (context.l10n.download_count(count), ExtisIcons.download),
    "add-to-playlist" => (
        context.l10n.add_count_to_playlist(count),
        ExtisIcons.playlistAdd
      ),
    "add-to-queue" => (
      ...
      return SurfaceCard(
        child: Basic(
          ...
          trailing: IconButton.ghost(
            ...
            icon: const Icon(ExtisIcons.close),
            onPressed: () {
              overlay.close();
            },
          ),
  ...
  Widget build(BuildContext context, ref) {
    ...
    Future<void> actionDownloadTracks({
      ...
      final fullTrackObjects =
          tracks.whereType<ExtisFullTrackObject>().toList();
    ...
    return AdaptivePopSheetList(
          ...
          case "play-next":
            {
              ...
              if (options.collection is ExtisSimpleAlbumObject) {
                historyNotifier.addAlbums(
                    [options.collection as ExtisSimpleAlbumObject]);
              } else {
                historyNotifier.addPlaylists(
                    [options.collection as ExtisSimplePlaylistObject]);
              }
              notifier.deselectAllTracks();
              if (!context.mounted) return;
              showToastForAction(context, action, tracks.length);
      ...
      icon: const Icon(ExtisIcons.moreVertical),
      variance: ButtonVariance.outline,
      items: (context) => [
        AdaptiveMenuButton(
          value: "download",
          leading: const Icon(ExtisIcons.download),
