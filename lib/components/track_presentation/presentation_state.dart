...
class PresentationState {
  final List<ExtisTrackObject> selectedTracks;
  final List<ExtisTrackObject> presentationTracks;
  final SortBy sortBy;
  ...
  PresentationState copyWith({
    List<ExtisTrackObject>? selectedTracks,
    List<ExtisTrackObject>? presentationTracks,
    SortBy? sortBy,
  }) {
...
class PresentationStateNotifier
  ...
  PresentationState build(collection) {
    if (arg case ExtisSimplePlaylistObject() || ExtisSimpleAlbumObject()) {
      if (isSavedTrackPlaylist) {
        ...
        ref.listen(
          arg is ExtisSimplePlaylistObject
              ? metadataPluginPlaylistTracksProvider(
                  (arg as ExtisSimplePlaylistObject).id)
              : metadataPluginAlbumTracksProvider(
                  (arg as ExtisSimpleAlbumObject).id),
          (previous, next) {
            next.whenData((value) {
              state = state.copyWith(
                presentationTracks: ServiceUtils.sortTracks(
  ...
  bool get isSavedTrackPlaylist =>
      arg is ExtisSimplePlaylistObject &&
      (arg as ExtisSimplePlaylistObject).id == "user-liked-tracks";

  List<ExtisTrackObject> get tracks {
    assert(
      arg is ExtisSimplePlaylistObject || arg is ExtisSimpleAlbumObject,
      "arg must be ExtisSimplePlaylistObject or ExtisSimpleAlbumObject",
    );

    final isPlaylist = arg is ExtisSimplePlaylistObject;
  ...
  void selectTrack(ExtisTrackObject track) {
  ...
  void deselectTrack(ExtisTrackObject track) {
    state = state.copyWith(
