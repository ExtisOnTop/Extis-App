...
class WindowsBuildCommand extends Command with BuildCommandCommonSteps {
  ...
  void run() async {
    ...
    final chocoFiles = [
      ...
      join(cwd.path, "choco-struct", "extis.nuspec"),
    ];
    ...
    runnerRCFile.writeAsStringSync(
      runnerRCFile
          .readAsStringSync()
          .replaceAll("%{{EXTIS_VERSION}}%", versionWithoutBuildNumber)
          .replaceAll(
            "%{{EXTIS_VERSION_AS_NUMBER}}%",
            [
    ...
    final ogExe = File(
      join(
        ...
        "extis-${pubspec.version}-windows-setup.exe",
      ),
    );

    final exePath = join(cwd.path, "dist", "Extis-windows-x86_64-setup.exe");
    ...
    final chocoNupkg = File(
      join(cwd.path, "dist", "extis.$versionWithoutBuildNumber.nupkg"),
    );

    final distNupkgPath = join(
      cwd.path,
      "dist",
      "Extis-windows-x86_64.nupkg",
    );
